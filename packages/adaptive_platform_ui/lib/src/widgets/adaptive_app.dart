...
class AdaptiveApp extends StatelessWidget {
  ...
  Widget _buildCupertinoApp(
    ...
    //
    // ФОРК: порядок обёрток обратный оригинальному. Раньше сначала работал
    // `wrapWithThemeMode`, а пользовательский builder оборачивал уже готовое
    // поддерево — и созданный внутри `MediaQuery` (он нужен для
    // platformBrightness) оказывался НИЖЕ всего, что поставил builder.
    // Любые правки MediaQuery со стороны приложения — например
    // `MediaQuery.withClampedTextScaling`, ограничивающий системный масштаб
    // шрифта, — этот внутренний MediaQuery затирал: он копирует данные из
    // контекста выше builder'а.
    //
    // Теперь builder оборачивает сам Router, а тема ложится поверх: и
    // `CupertinoTheme`, и правки MediaQuery со стороны приложения доходят до
    // страниц.
