    ...
    private func handleMethodCall(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "setSelectedIndex":
            ...
            if let bar = self.tabBar, let items = bar.items, idx >= 0, idx < items.count {
                // ФОРК: присваивание selectedItem не анимируется, поэтому
                // стеклянный индикатор перескакивал на новую вкладку мгновенно.
                // Тап по панели забирает слой AppNativeTabBar на стороне
                // Flutter (иначе одиночное нажатие вообще не срабатывает), так
                // что нативная вью узнаёт о выборе только отсюда — своей
                // анимации у неё в этом сценарии не остаётся.
                //
                // Layout выполняем внутри блока анимации: только тогда UIKit
                // интерполирует новое положение индикатора вместо мгновенной
                // перекладки. Пружина близка к системной анимации UITabBar.
                // `beginFromCurrentState` — чтобы быстрые тапы подхватывали
                // недоигранную анимацию, а не начинали с нуля.
